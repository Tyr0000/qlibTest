...
{
public:
	DECLARE_DYNCREATE(CFListCtrl)
//	 DECLARE_DYNAMIC(CFListCtrl, CListCtrl)
...
//	TCHAR GetFItemText(HFROW hFRow, int iFColumn);
	//	устанавливает текст 
	void SetFItemText(HFROW hFRow, int iFColumn, CString strText, BOOL UpdateNow= FALSE);	
	void SetFItemVal(HFROW hFRow, int iFColumn, int Val, BOOL UpdateNow= FALSE);
	void SetFItenValText(HFROW hFRow, int iFColumn, int Val, CString strText, BOOL UpdateNow= FALSE);
	int GetFItemVal(HFROW hFRow, int iFColumn);
...
protected:		//	IMPLEMENTATION
	...
	HIDE*/
	static int DefaultCompareFRows(CFListCtrl* pThis, int iFColumn, HFROW hFRow1, HFROW hFRow2)
		{
...
protected:		//	IMPLEMENTATION
	...
	public:
		...
		HIDE*/ 
		// !!!!
		 static void InitFItem(XFList_Item &item)
		{
			item.m_dwUserData=0;
		...
		HIDE*/ CArray<XFList_Item, XFList_Item> m_aFItems;		//жлементы строки
...
protected:		//	IMPLEMENTATION
	...
	HIDE*/ inline XFList_Row::XFList_Item& GetFItem(HFROW hFRow, int iFColumn)
		{			
		#ifdef _DEBUG
		  XFList_Row::XFList_Item s_debugFItem;			
...
// Overrides
	// ClassConstructor generated virtual function overrides
