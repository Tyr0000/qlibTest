...
//
 ...
 IMPLEMENT_DYNCREATE(CFListCtrl, CFListCtrl) 
// IMPLEMENT_DYNAMIC(CFListCtrl, CFListCtrl) 

CFListCtrl::CFListCtrl()
{
//	EXIT_IF_NOT_AVAILABLE
...
// !!!! 
int CFListCtrl::GetRowCnt()
{
	return m_lstFRows.GetCount();
}

HFROW CFListCtrl::GetRowByInd(int Ind)
{
	return m_lstFRows.FindIndex(Ind);
}

//	вставляет новую строку в дерево-список
HFROW CFListCtrl::InsertFRow(HFROW hParent, HFROW hPosition, CString strText, int iImageIndex, int iControlProfile)
{
	int i;
	...
	XFList_Row::XFList_Item item;
	XFList_Row::InitFItem(item);
	for(i=0; i<m_aFColumns.GetSize(); i++){		
...
//TCHAR CFListCtrl::GetFItemText(HFROW hFRow, int iFColumn) 
{	
	return GetFItem(hFRow, iFColumn).m_strText;
}

//	устанавливает текст 	
void CFListCtrl::SetFItemText(HFROW hFRow, int iFColumn, CString strText, BOOL UpdateNow) 
{				
...
void CFListCtrl::SetFItemVal(HFROW hFRow, int iFColumn, int Val, BOOL UpdateNow)
{
TCHAR tStr[128];

	_itot(Val, tStr, 10);	
	GetFItem(hFRow, iFColumn).m_strText= tStr;
// !!!!
	if (IsFRowVisible(hFRow))
		UpdateFRow(hFRow);
}

void CFListCtrl::SetFItenValText(HFROW hFRow, int iFColumn, int Val, CString strText, BOOL UpdateNow)
{
TCHAR tStr[128];
...
int CFListCtrl::GetFItemVal(HFROW hFRow, int iFColumn)
{
	return _tstoi(GetFItem(hFRow, iFColumn).m_strText);
}
...
// hBr= CreateSolidBrush(::GetSysColor(COLOR_3DFACE) ); // 			COLOR_BTNFACE  

RECT rcClient;
			::GetClientRect(m_hWnd, &rcClient);
			
			::FillRect(pDC->m_hDC, &rcClient, (HBRUSH)::GetStockObject(LTGRAY_BRUSH));
//			hBr= ::CreateSolidBrush(::GetSysColor(COLOR_3DFACE));
			hPen= (HPEN)::SelectObject(pDC->m_hDC, (HPEN)::GetStockObject(NULL_PEN)); 
...
int CFList_ComboBox::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	int select=-1;
	FL_COMBOITEM *pci;
	for(i= 0; i<m_paComboItems->GetSize(); i++)
	{
